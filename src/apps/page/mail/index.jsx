...
class Mail extends Component {
  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.props.updateOver({over:[1,this.props.over[0]]})
      }
    });
  }
  ...
  render() {
    ...
    const suffix = <Icon type="eye" theme="filled" style={{color: this.state.eye ? '#007FFF ' : '#B7BBC2'}}
                         onClick={this.eye}/>;
                ...
                >
                  {getFieldDecorator('password', {
                    ...
                    <Input size="large" type={this.state.eye ? 'text' : 'password'} suffix={suffix}
                           placeholder="大写字母、小写字母、数字、符号至少包含三种"/>
                  )}
                </FormItem>
              </div>
            </div>
